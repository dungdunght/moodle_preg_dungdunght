...
abstract class block_formal_langs_abstract_language {
    ...
    /**
     * String with default determiner for language.
     * @var string with default delimiter for language
     */
    ...
    /**
     * Returns default delimiter for language.
     *
     * @return string
     */
     public function token_delimiter() {
    ...
    public function parse($processedstring, $iscorrect) {
        ...
        if ($iscorrect) {
            $this->validate($processedstring);
        }
    }

    /**
     * Performs validation of language constructs for language. Fills errors fields for language
     * with errors, if any errors are occurred
     * @param block_formal_langs_processed_string $processedstring
     */
    protected function validate($processedstring) {
     ...
     */
     public function create_from_string($string, $classname = 'block_formal_langs_processed_string') {
        ...
        $sourcestring = $string;
        if (!is_object($string)) {
            $sourcestring = new qtype_poasquestion\utf8_string($string);
        }
...
abstract class block_formal_langs_predefined_language extends block_formal_langs_abstract_language {
    ...
    public function scan(&$processedstring) {
        ...
        if (is_a($string,'qtype_poasquestion\utf8_string') == true) {
